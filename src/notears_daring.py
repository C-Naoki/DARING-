...
class NotearsMLP(nn.Module):
    def __init__(self, dims, bias=True):
        ...
        layers = []
        for i in range(len(dims) - 2):  # extend to MLP
...
def squared_loss(output, target, _I=0, J=0):
    if _I == 0:
...
def notears_nonlinear(
    ...
    for _ in range(max_iter):
        rho, alpha, h = dual_ascent_step(
            model, X, lambda1, lambda2, lambda3, rho, alpha, h, rho_max
        )
...
def main():
    ...
    parser.add_argument(
        "--data_path", type=str, default=None, help="Path to data files"
    )
    parser.add_argument("--d", type=int, default=20, help="variable")
    ...
    parser.add_argument(
        "--penalty", type=int, default=10, help="coefficient of regularizer"
    )
    ...
    for trial in range(opt.trials):
        B_true = np.load(f"{opt.data_path}_{trial}.npy", allow_pickle=True).item()[
            "graph"
        ]
        X = np.load(f"{opt.data_path}_{trial}.npy", allow_pickle=True).item()["data"]
        if not isinstance(B_true, list):
            B_true = list(B_true)
        ...
        W_est = notears_nonlinear(
            model, X, lambda1=opt.lambda1, lambda2=opt.lambda2, lambda3=opt.penalty
        )

        _shd = []
        _sid = []
        for _B in B_true:
            acc = metric.count_accuracy(_B != 0, W_est != 0)
            # print(acc["shd"], acc["sid"])
            _shd.append(acc["shd"])
            _sid.append(acc["sid"])
        print(f"seed{trial} ===> SHD: {np.mean(_shd):.2f}, SID: {np.mean(_sid):.2f}")
        shd += _shd
        sid += _sid
    print(f"avg SHD: {np.mean(shd):.2f}, avg SID: {np.mean(sid):.2f}")

    with open(
        f"out/{opt.data_path.split('/')[-1]}/{opt.lambda1}_{opt.lambda2}_{opt.penalty}.json",
        "w",
    ) as f:
        json.dump(
            {
                "shd": {
                    "values": [f"{value:.2f}" for value in shd],
                    "mean": f"{np.mean(shd):.2f}",
                },
                "sid": {
                    "values": [f"{value:.2f}" for value in sid],
                    "mean": f"{np.mean(sid):.2f}",
                },
            },
            f,
            indent=2,
